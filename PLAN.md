...
**Week 1 (Build):** Build performance monitor (tracks skill success rates, tool selection accuracy, benchmark scores over time). Current SPK-001 implements the read-only performance monitor snapshot contract via `evolution.monitor.performance_snapshot`; it consumes sanitized aggregate metrics, writes only under `output/phase5-continuous-loop/<run-id>/`, and routes weak areas to manual triage; no optimizer is started. Current SPK-002 implements read-only auto-triage ranking via `evolution.monitor.auto_triage`; it ranks sanitized performance snapshot weak areas by impact × frequency for manual review; no optimizer is started. Current SPK-003 implements a read-only scheduler dry-run via `evolution.monitor.scheduler_dry_run`; it converts auto-triage targets into manual-review dry-run actions only, emits a local candidate bundle queue, can consume existing bundle `decision.json` evidence, and no cron jobs are created. Current SPK-004 implements a supervised local candidate runner via `evolution.monitor.supervised_candidate_runner`; it requires the public approval sentinel `APPROVE_LOCAL_CANDIDATE_RUNNER`, handles exactly one approved queue target, re-consumes an existing local bundle `decision.json`, rejects inline command execution in this safety slice, and keeps active apply/GitHub publication/deployment disabled. Wire up to Hermes cron scheduler only after separate approval.
