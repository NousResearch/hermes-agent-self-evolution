...
**Week 1 (Build):** Build performance monitor (tracks skill success rates, tool selection accuracy, benchmark scores over time). Current SPK-001 implements the read-only performance monitor snapshot contract via `evolution.monitor.performance_snapshot`; it consumes sanitized aggregate metrics, writes only under `output/phase5-continuous-loop/<run-id>/`, and routes weak areas to manual triage; no optimizer is started. Build auto-triage logic (ranks optimization targets by impact × frequency). Wire up to Hermes cron scheduler only after separate approval.
