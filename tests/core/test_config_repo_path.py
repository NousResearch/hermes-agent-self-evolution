...
def _set_home(monkeypatch, path) -> None:
    """Point the user's home at ``path`` on every platform.

    ``Path.expanduser()`` reads ``HOME`` on POSIX and ``USERPROFILE`` on
    Windows, so a test that sets only ``HOME`` passes on CI and fails on a
    Windows checkout for reasons unrelated to the code being tested.
    Reported upstream in NousResearch/hermes-agent-self-evolution#178.
    """
...
def test_resolve_honors_explicit_path_without_default(tmp_path, monkeypatch):
    ...
    _set_home(monkeypatch, tmp_path / "empty")  # no default location
...
def test_resolve_expands_user_home(monkeypatch):
    ...
    _set_home(monkeypatch, "/home/example")
...
def test_config_constructs_without_repo(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HERMES_AGENT_REPO", raising=False)
    _set_home(monkeypatch, tmp_path / "empty")
