...
## Troubleshooting (hard-won lessons from 2026-04-13)

### Model Output Quirks

**MiMo-v2-pro (Nous free tier):** Output goes in `reasoning` field, not `content`. DSPy expects `content`. Result: judge returns default scores, zero variance across trials, MAD=0 everywhere. Not usable for confidence scoring.

**GPT-5.4 via hermes chat:** Outputs table format (| Score | Weight |) not structured lines ("correctness: 0.X"). Parser must handle both — regex fallback to extract first 3 floats from table if structured parsing fails.

**Nous API instability:** Can hang 10+ minutes on single calls. Don't kill prematurely — process usually completes eventually. Check TCP connections via `lsof -p PID -n` to confirm API is alive.

### Confidence Math Gotchas

**compute_confidence expects raw scores, not deltas.** If you pass [e1-b1, e2-b2, ...] (deltas), the function treats the first delta as "baseline" and the max as "best" — math is wrong. For holdout confidence, use direct MAD on deltas:

```python
delta_scores = [e - b for e, b in zip(evolved_scores, baseline_scores)]
mean_delta = sum(delta_scores) / len(delta_scores)
mad = compute_mad(delta_scores)
confidence = abs(mean_delta) / mad if mad > 0 else abs(mean_delta) / 0.01
```

**Per-example confidence requires scoring variance.** If the judge returns identical scores across 3 trials (same input, same model), MAD=0 and all labels are "within noise". Use a model with temperature sensitivity (GPT-5.4 works, MiMo doesn't).

### Constraint Validator

**Validate full skill text, not body-only.** evolve_skill.py was validating `evolved_body` (no frontmatter) — always fails `skill_structure` check. Fix: validate `evolved_full = reassemble_skill(frontmatter, evolved_body)` and pass `skill["raw"]` as baseline for growth comparison.

### Holdout Eval Performance

**hermes chat subprocess = 60-120s per call.** 5 examples × 3 trials × 2 variants = 30 calls = 30-60 minutes. For fast iteration, use single-score holdout (no MAD trials) during development, MAD trials only for final validation.

**Direct API calls (2-5s) would solve this** but need separate API key management. HermesJudge trades speed for auth simplicity.

### MIPROv2 Optimization

**GEPA not available in current DSPy** — falls back to MIPROv2 automatically. Works fine but is slower (10 trials vs GEPA's adaptive search).

**Optimized instructions are not saved by default.** Code must explicitly extract and save `optimized_module.signature.instructions` and `optimized_module.demos` to proof.json. Without this, the "improvement" has no evidence.

**Near-optimal baselines (>95%) produce noise results.** MIPROv2 can't improve what's already good. MAD correctly catches this — "within noise" label prevents false positives.
...
├── evolved_skill.md     # Evolved skill text (may be identical)
├── metrics.json         # Confidence data, per-example labels
└── proof.json           # MIPROv2 instructions, demos, skill_text_changed flag
```
